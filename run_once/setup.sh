...
printf "    3. ${BOLD}Pi-hole:${NC} Settings → DNS → Custom 1 → ${DIM}172.20.0.1#5335${NC}\n"
printf "       ${DIM}To verify: ${DIM}docker inspect -f '{{range .NetworkSettings.Networks}}{{.Gateway}}{{end}}' pihole${NC}\n"
printf "    4. ${BOLD}Borg:${NC} borg key import /mnt/external_hdd/borg-repo ~/borg-key-backup.txt → borgmatic check\n"
