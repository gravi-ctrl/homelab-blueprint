...
#   2. Decrypt & Extract:   sudo age -d -i /root/.backup-key.txt docker-stacks-*.tar.zst.age | sudo tar --zstd -xf - -C /
...
timeout 60m tar --use-compress-program="zstd -9 -T0 --long" -cf - \
    ...
    --exclude="${STACKS_DIR#/}/nextcloud/html/*.php" \
    --exclude="${STACKS_DIR#/}/nextcloud/html/*.json" \
    --exclude="${STACKS_DIR#/}/nextcloud/html/*.lock" \
    --exclude="${STACKS_DIR#/}/nextcloud/html/.*" \
