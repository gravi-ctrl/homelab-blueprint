...
# @FREQUENCY: Weekly 5:30am on Thursday
# @USES_ENV: BACKUP_DIR, STACKS_DIR, AGE_KEYFILE, KUMA_HC_URL, CTRL_DIR
# @CRON: root
# ==============================================================================
# RESTORE:
#   1. Stop Docker:         sudo systemctl stop docker
#   2. Decrypt & Extract:   sudo age -d -i /root/.backup-key.txt docker-stacks-*.tar.zst.age | sudo tar --zstd --same-owner --numeric-owner --transform="s,^home/[^/]\+,${HOME#/}," -xf - -C /
...
# --- .ENV ---
if [ -f "/opt/rabbit-hole/.env" ]; then
    source "/opt/rabbit-hole/.env"
else
    echo "❌ .env file not found in /opt/rabbit-hole"
    exit 1
fi
