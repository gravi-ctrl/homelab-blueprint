...
```
The lock auto-releases when the fd closes at script exit — no explicit unlock needed.

**Python** (`git-auto-sync.py`): same idea via `fcntl.flock(..., LOCK_EX | LOCK_NB)` on Linux,
`msvcrt.locking()` on Windows — branched once on `sys.platform`, same OS-conditional shape as
`cron-guard.py`'s process-tree kill. Acquired right after `chdir` into the target repo, held
for the entire run via `try`/`finally`, released on every exit path including `sys.exit()`
(which raises `SystemExit` — `finally` still runs).

**Why `-n` / `LOCK_NB` (fail immediately) rather than waiting:** `gitops-deploy.sh` re-runs
every 15 minutes regardless, so skipping one cycle and catching the deploy next time is free.
A multi-second wait would also eat into whatever timeout `cron-guard` has configured for that
job, for no benefit.

**Why the lock file lives at `<repo>/.git/sync.lock` specifically:** it makes the convention
apply to any future script without a registry to remember — the rule is just "touching this
repo's git state? Lock its own `.git/sync.lock` first."
...
- Writing any new script that runs `git checkout`/`pull`/`push` against a shared repo? Take
  the `.git/sync.lock` first — see §8. Don't fall back to schedule-staggering.
