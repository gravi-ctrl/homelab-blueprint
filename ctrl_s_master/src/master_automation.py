...
def run_command(command_list, is_python_script=False, working_dir=None):
    ...
    try:
        ...
        # Capture FULL output for the email body
...
def send_email_report(job_succeeded, failed_task=None, failure_output=None, generate_log_only=False):
    if not generate_log_only: print("\n" + "="*70); print("--- Task: Sending Email Report ---")
    if not all([EMAIL_HOST, EMAIL_PORT, EMAIL_SENDER, EMAIL_PASSWORD, EMAIL_RECIPIENT]): return

    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    subject = f"{'✅' if job_succeeded else '❌'} Automation Report: {timestamp}"
    
    html_body = f"<h2>Run Status: {'<span style="color:green">Success</span>' if job_succeeded else '<span style="color:red">Failed</span>'}</h2>"
    
    if failed_task:
        html_body += f"<h3>⚠️ Failed Task: {failed_task}</h3>"
        if failure_output:
            # Styling to match the Windows "Code Block" look
            html_body += f"""
            <div style="background-color: #f8d7da; border: 1px solid #f5c6cb; padding: 10px; border-radius: 5px; font-family: monospace; color: #721c24;">
                <strong>Error Output:</strong><br>
                <pre style="white-space: pre-wrap;">{html.escape(failure_output)}</pre>
            </div>
            """

    files_to_send = []
    if STATUS_DASHBOARD_FILE.exists(): files_to_send.append(STATUS_DASHBOARD_FILE)
    log_patterns = os.getenv("EMAIL_ATTACH_LOGS", "")
    if log_patterns:
        for pat in log_patterns.split(','):
            logs = list(LOGS_DIR.glob(pat.strip()))
            if logs: files_to_send.append(sorted(logs, key=os.path.getmtime, reverse=True)[0])

    if not generate_log_only:
        try:
            msg = EmailMessage(); msg['Subject'], msg['From'], msg['To'] = subject, EMAIL_SENDER, EMAIL_RECIPIENT
            msg.set_content("HTML required."); msg.add_alternative(html_body, subtype='html')
            for fpath in files_to_send:
                with open(fpath, 'rb') as f: msg.add_attachment(f.read(), maintype='text', subtype='plain', filename=fpath.name)
            with smtplib.SMTP_SSL(EMAIL_HOST, int(EMAIL_PORT)) as server:
                server.login(EMAIL_SENDER, EMAIL_PASSWORD); server.send_message(msg)
            print("Email sent.")
    ...
    if args.command == 'run-tasks':
        ...
        for task_func in task_list:
            ...
            if not success:
                ...
                failed_task = task_func.__name__; failure_output = output
        ...
        send_email_report(args.status == 'success', generate_log_only=args.generate_log_only)
