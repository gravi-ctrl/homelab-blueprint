...
def fetch_all_bookmarks(api_token):
    ...
    while True:
        try:
            ...
            print(f"ERROR: An error occurred during API request: {e}"); return None, -1
...
def save_as_json(bookmarks, filepath: Path):
    try:
        ...
        print(f"OK: Successfully saved JSON file.")
    except IOError as e: print(f"ERROR: Error saving JSON file: {e}")
...
def save_as_csv(bookmarks, filepath: Path, collection_map):
    try:
        ...
        print(f"OK: Successfully saved CSV file.")
    except IOError as e: print(f"ERROR: Error saving CSV file: {e}")
    ...
    try:
        with filepath.open('w', encoding='utf-8') as f:
            ...
            for item in bookmarks:
                ...
                f.write('</li>\n')
            f.write("</ul>\n" + html_footer)
        print(f"OK: Successfully saved HTML file.")
    except IOError as e: print(f"ERROR: Error saving HTML file: {e}")
...
def process_account_backup(account_config, config):
    ...
    if total_api_count != -1 and len(bookmarks) != total_api_count:
        print(f"  [WARNING] Mismatch detected! The script may not have fetched all bookmarks.")
    else:
        print(f"  [OK] Downloaded count matches API count.")
    ...
    try:
        with zipfile.ZipFile(zip_file_path, 'w', zipfile.ZIP_DEFLATED) as zf:
            for file in files_to_zip: zf.write(file, file.name)
        print(f"OK: Successfully created ZIP archive.")
        print("Cleaning up individual backup files...")
        for file in files_to_zip: file.unlink()
    except (IOError, zipfile.BadZipFile) as e:
        print(f"ERROR: Could not create or write to ZIP file: {e}"); return False
    print("\n    --- Starting Cleanup ---")
