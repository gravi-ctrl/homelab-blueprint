...
def get_keys(data, passphrase):
    if not (data.get("encrypted") and data.get("passwordProtected")):
        sys.exit("Error: Input: not encrypted or account protected!")
    try:
        ...
        if data["kdfType"] == 0:
            ...
            try: import argon2
            except ImportError: sys.exit("Error: Install argon2-cffi (pip install argon2-cffi)")
            digest = hashes.Hash(hashes.SHA256()); digest.update(salt); salt_hash = digest.finalize()
            key = argon2.low_level.hash_secret_raw(passphrase, salt=salt_hash, time_cost=data.get("kdfIterations", DEFAULT_ARGON2_PARAMS['iterations']), memory_cost=data.get("kdfMemory", DEFAULT_ARGON2_PARAMS['memory']) * 1024, parallelism=data.get("kdfParallelism", DEFAULT_ARGON2_PARAMS['parallelism']), hash_len=32, type=argon2.low_level.Type.ID)
...
def decrypt(inp, enc_key, mac_key):
    try:
        ...
        sys.exit(f"Error during decryption: {e}. Check password or for file corruption.")
...
def import_bitwarden_data(kp, bw_data):
    ...
    for item in bw_data.get('items', []):
        ...
        try:
            ...
            title, username, password, url, totp, original_notes = item.get('name') or 'No Title', '', '', '', '', item.get('notes') or ''
            password_history, notes_parts, extra_uris = [], [], []
            if item.get('type') == 1:
                login = item.get('login', {})
                username, password, totp = login.get('username') or '', login.get('password') or '', login.get('totp')
            ...
            elif item.get('type') == 3: notes_parts.append("--- Credit Card Details ---\n" + "\n".join(f"{k}: {v}" for k,v in item.get('card', {}).items()))
            elif item.get('type') == 4: notes_parts.append("--- Identity Details ---\n" + "\n".join(f"{k}: {v}" for k,v in item.get('identity', {}).items()))
            if original_notes:
            ...
            def create_string_element(key, value, protected=False):
                ...
                if value is not None: value_el.text = value
                if protected: value_el.set('Protected', 'True')
                return string_el
            ...
            if totp:
                entry_element.append(create_string_element('otp', f"otpauth://totp/Default?secret={totp.replace(' ', '')}", protected=True))
            times_element = etree.Element('Times')
            now_iso = datetime.now(UTC).strftime('%Y-%m-%dT%H:%M:%SZ')
            if item.get('revisionDate'):
                try: etree.SubElement(times_element, 'LastModificationTime').text = date_parse(item['revisionDate']).strftime('%Y-%m-%dT%H:%M:%SZ')
                except: pass
            etree.SubElement(times_element, 'CreationTime').text = date_parse(item['creationDate']).strftime('%Y-%m-%dT%H:%M:%SZ') if item.get('creationDate') else now_iso
            ...
            if password_history:
                ...
                for hist_item in password_history:
                    ...
                    hist_entry_element.append(create_string_element('Password', hist_item.get('password'), protected=True))
...
def main():
    ...
    if len(sys.argv) < 2: sys.exit("ERROR: First argument must be the FULL PATH to the json file!")
    ...
    if vaults_dir_override:
        kdbx_dir = Path(vaults_dir_override) / "kdbx"
    else:
        kdbx_dir = project_root / "vaults" / "kdbx"
    ...
    try:
        ...
        else: sys.exit(f"FATAL: Could not determine vault type from filename: {input_full_path.name}")
        
        if not bw_pass or not kdbx_pass: sys.exit("FATAL: Required password environment variables not found.")
        ...
        if skipped_items:
            ...
            for item in skipped_items: print(f"  - Item Name: {item['name']}\n    Error: {item['error']}\n")
