...
.task-row { position: relative; padding-left: 18px; margin-bottom: 16px; font-size: 14px; color: var(--tx); line-height: 1.7; }
.task-row:last-child { margin-bottom: 0; }
.task-row::before { content: "•"; position: absolute; left: 0; top: 0; color: var(--tx3); font-size: 18px; }

.mono-script { font-family: ui-monospace, monospace; font-size: 12.5px; background: var(--bg3); padding: 2px 6px; border-radius: 4px; color: var(--tx); font-weight: 600; border: .5px solid var(--br2); }
.time-badge { font-weight: 500; color: var(--blue-tx); background: var(--blue-bg); padding: 2px 6px; border-radius: 4px; font-size: 11px; display: inline-block; margin-left: 4px; letter-spacing: 0.01em; white-space: nowrap; vertical-align: middle; }
  ...
  Object.keys(grouped).sort().forEach(category => {
    ...
    scripts.forEach(s => {
      ...
      narrativeHtml += `
          <div class="task-row">
            ${warnIcon}
            <code class="mono-script">${e(s.name)}</code>
